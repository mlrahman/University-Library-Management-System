...
if($_POST['keyword']!="")
{
	...
	foreach ($list as $rs) {
		...
		// add new option
		echo '<li onclick="set_item(\''.str_replace("'", "\'", $rs['name']).'\')" style="width:500px;"><a href="u_user.php?search=act&neub_id_no='.$rs['neub_id_no'].'" style="text-decoration:none;"><img src="images/user/'.$rs['picture'].'" height="40" width="30" style="float:left;border-radius:5px;"><p style="margin-left:10px;float:left;font-size:15px;padding-top:5px;color:blue;">'.$user_name.'</p></a></li></br></br>';
	}
	if($fl==0)
